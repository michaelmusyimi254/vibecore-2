...
const RoleSpecificStep: React.FC<RoleSpecificStepProps> = ({ role, onNext, onBack }) => {
  ...
  // Autosave logic placeholder
  const onSubmit = (data: any) => {
    // TODO: POST to /api/profiles/update
    onNext();
  };
      ...
      content = (
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <select {...register('experienceLevel')} className="w-full border rounded p-2">
          ...
          <div className="mb-4">
            ...
            <input className="w-full border rounded p-2 mb-2" placeholder="Full Name" {...register('emergencyContactName')} />
            <input className="w-full border rounded p-2 mb-2" placeholder="Phone Number" {...register('emergencyContactPhone')} />
            <input className="w-full border rounded p-2" placeholder="Relationship" {...register('emergencyContactRelation')} />
          </div>
          <div className="mb-4">
      ...
      content = (
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <textarea className="w-full border rounded p-2" placeholder="Tell us about your coaching experience..." {...register('bio')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Specialties</label>
            {COACH_SPECIALTIES.map(spec => (
              <label key={spec} className="block text-sm">
                <input type="checkbox" value={spec} {...register('specialties')} /> {spec}
              </label>
            ))}
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Certifications</label>
            <textarea className="w-full border rounded p-2" placeholder="List your certifications..." {...register('certifications')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Years of Experience</label>
            <input type="number" className="w-full border rounded p-2" placeholder="Years" {...register('yearsOfExperience')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Hourly Rate (USD)</label>
            <input type="number" className="w-full border rounded p-2" placeholder="Hourly Rate" {...register('hourlyRate')} />
          </div>
          <div className="mb-4">
      ...
      content = (
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <input className="w-full border rounded p-2" placeholder="Facility Name" {...register('facilityName')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Facility Type</label>
            <select {...register('facilityType')} className="w-full border rounded p-2">
          ...
          <div className="mb-4">
            ...
            <textarea className="w-full border rounded p-2" placeholder="Describe your facility..." {...register('description')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Amenities</label>
            {FACILITY_AMENITIES.map(amenity => (
              <label key={amenity} className="block text-sm">
                <input type="checkbox" value={amenity} {...register('amenities')} /> {amenity}
              </label>
            ))}
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Business Hours</label>
            <input className="w-full border rounded p-2" placeholder="e.g. Mon-Fri 6am-10pm, Sat-Sun 8am-8pm" {...register('businessHours')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Capacity</label>
            <input type="number" className="w-full border rounded p-2" placeholder="Maximum Capacity" {...register('capacity')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Location</label>
            <input className="w-full border rounded p-2 mb-2" placeholder="Address" {...register('address')} />
            <input className="w-full border rounded p-2 mb-2" placeholder="City" {...register('city')} />
            <input className="w-full border rounded p-2 mb-2" placeholder="State/Province" {...register('state')} />
            <input className="w-full border rounded p-2 mb-2" placeholder="ZIP/Postal Code" {...register('zipCode')} />
            <input className="w-full border rounded p-2" placeholder="Country" {...register('country')} />
          </div>
          <div className="mb-4">
      ...
      content = (
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <input className="w-full border rounded p-2" placeholder="Business Name" {...register('businessName')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Business Type</label>
            <select {...register('businessType')} className="w-full border rounded p-2">
          ...
          <div className="mb-4">
            <label className="block font-medium mb-1">Description</label>
            <textarea className="w-full border rounded p-2" placeholder="Describe your brand..." {...register('businessDescription')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Product Categories</label>
            {PRODUCT_CATEGORIES.map(cat => (
              <label key={cat} className="block text-sm">
                <input type="checkbox" value={cat} {...register('productCategories')} /> {cat}
              </label>
            ))}
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Tax ID / VAT Number</label>
            <input className="w-full border rounded p-2" placeholder="Tax ID / VAT Number" {...register('taxId')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Business Address</label>
            <input className="w-full border rounded p-2 mb-2" placeholder="Street Address" {...register('address')} />
            <input className="w-full border rounded p-2 mb-2" placeholder="City" {...register('city')} />
            <input className="w-full border rounded p-2 mb-2" placeholder="State/Province" {...register('state')} />
            <input className="w-full border rounded p-2 mb-2" placeholder="ZIP/Postal Code" {...register('zipCode')} />
            <input className="w-full border rounded p-2" placeholder="Country" {...register('country')} />
      ...
      content = (
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <input className="w-full border rounded p-2" placeholder="Organization Name" {...register('organizationName')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Organization Type</label>
            <input className="w-full border rounded p-2" placeholder="Organization Type" {...register('organizationType')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Description</label>
            <textarea className="w-full border rounded p-2" placeholder="Describe your organization..." {...register('description')} />
          </div>
          <div className="mb-4">
            <label className="block font-medium mb-1">Event Types</label>
            {EVENT_TYPES.map(type => (
              <label key={type} className="block text-sm">
                <input type="checkbox" value={type} {...register('eventTypes')} /> {type}
              </label>
            ))}
          </div>
          <div className="mb-4">
